  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="text-right">
        ...
        <span className={`mt-1.5 inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium ${isOnline
            ? "border-emerald-800/60 bg-emerald-950/60"
            : "border-red-800/60 bg-red-950/60 text-red-400"
          }`}
          style={isOnline ? { color: "#2abe9bff" } : {}}>
          <span className={`inline-block h-1.5 w-1.5 rounded-full ${isOnline ? "animate-pulse bg-emerald-400" : "bg-red-500"
            }`} />
          {deviceId}
        </span>
      </div>
